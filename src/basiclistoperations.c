 ...
 */
PLAYERCARD *
playercard_create (int pId, char *name, int rating, int title,
                   int expectedRounds)
{
  ...
  if ((newPlayerCard->opponents
       = (int *)malloc (sizeof (int) * expectedRounds))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERCARD);
  else
    memset (newPlayerCard->opponents, -1,
            sizeof (*(newPlayerCard->opponents)) * expectedRounds);

  if ((newPlayerCard->results
       = (char *)malloc (sizeof (char) * expectedRounds))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERCARD);
  else
    memset (newPlayerCard->results, ' ',
            sizeof (*(newPlayerCard->results)) * expectedRounds);

  if ((newPlayerCard->colors = (int *)malloc (sizeof (int) * expectedRounds))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERCARD);
  else
    memset (newPlayerCard->colors, 0,
            sizeof (*(newPlayerCard->colors)) * expectedRounds);

  if ((newPlayerCard->floaterhistory
       = (int *)malloc (sizeof (int) * expectedRounds))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERCARD);
  else
    memset (newPlayerCard->floaterhistory, 0,
            sizeof (*(newPlayerCard->floaterhistory)) * expectedRounds);
 ...
 */
PLAYERLISTENTRY *
playerlistentry_create (PLAYERCARD *aPlayerCard)
{
  PLAYERLISTENTRY *newPlayerListEntry;

  if ((newPlayerListEntry
       = (PLAYERLISTENTRY *)malloc (sizeof (PLAYERLISTENTRY)))
      == NULL)
 ...
 */
PAIRINGLISTENTRY *
pairinglistentry_create (PLAYERCARD *player1, PLAYERCARD *player2)
{
  PAIRINGLISTENTRY *newPairingListEntry;

  if ((newPairingListEntry
       = (PAIRINGLISTENTRY *)malloc (sizeof (PAIRINGLISTENTRY)))
      == NULL)
...
playeradministration_create ()
{
  PLAYERADMINISTRATION *newPlayerAdministration;

  if ((newPlayerAdministration
       = (PLAYERADMINISTRATION *)malloc (sizeof (PLAYERADMINISTRATION)))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERADMINISTRATION);
  else
    {
      if ((newPlayerAdministration->player_list = playerlist_create ())
          == NULL)
        fprintf (stderr, L_MEMERROR_PLAYERADMINISTRATION);
      else
        {
          if ((newPlayerAdministration->player_ht = (PLAYERCARD **)malloc (
 ...
 */
void
playercard_finalize (PLAYERCARD *aPlayerCard)
{
 ...
 */
void
playerlistentry_finalize (PLAYERLISTENTRY *aEntry)
{
  free (aEntry);
  aEntry = NULL;
}
 ...
 */
void
playerlist_clear (PLAYERLIST *aPlayerList)
{
 ...
 */
void
playerlist_finalize (PLAYERLIST *aPlayerList)
{
 ...
 */
void
pairinglistentry_finalize (PAIRINGLISTENTRY *aEntry)
{
  free (aEntry);
  aEntry = NULL;
}
 ...
 */
void
pairinglist_clear (PAIRINGLIST *aPairingList)
{
 ...
 */
void
pairinglist_finalize (PAIRINGLIST *aPairingList)
{
 ...
 */
void
playergroup_finalize (PLAYERGROUP *aPlayerGroup)
{
...
/**
 * alle Spielergruppe freigeben die durch verkettung an dieser (ersten) Gruppe
 * hängen finalize all playergroup, which are attached to this (first)
 * playergroup
 */
void
playergroup_finalizeall (PLAYERGROUP *firstPlayerGroup)
{
  ...
  if (aPlayerCard->pId >= aPlayerAdministration->size_ht)
    {
      ...
      if ((aPlayerAdministration->player_ht
           = (PLAYERCARD **)realloc (aPlayerAdministration->player_ht,
                                     sizeof (PLAYERCARD *) * nextSize))
          == NULL)
        {
 ...
 */
void
playerlist_append (PLAYERLIST *aPlayerList, PLAYERCARD *aPlayerCard)
{
...
/**
 * Loescht einen eintrag aus einer spielerliste (keine Ueberpruefung ob der
 * Eintrag auch zu dieser Liste gehoert!) deletes a entry form playerlist (no
 * check if the entry is part  of this list !!)
 */
void
playerlist_delete (PLAYERLIST *aPlayerList, PLAYERLISTENTRY *aPlayerListEntry)
{
 ...
 */
int
playerlist_remove (PLAYERLIST *aPlayerList, PLAYERCARD *aPlayerCard)
{
  PLAYERLISTENTRY *aPlayerListEntry;
 ...
 */
void
pairinglist_append (PAIRINGLIST *aPairingList, PLAYERCARD *player1,
                    PLAYERCARD *player2)
{
  PAIRINGLISTENTRY *aEntry;
 ...
 */
int
playergroup_remove (PLAYERGROUP *aPlayerGroup, PLAYERCARD *aPlayerCard)
{
 ...
 */
void
playeradministration_add (PLAYERADMINISTRATION *aPlayerAdministration,
                          PLAYERCARD *aPlayerCard)
{
 ...
 */
void
playergroup_add (PLAYERGROUP *aPlayerGroup, PLAYERCARD *aPlayerCard)
{
 ...
 */
void
playergroup_insertbehind (PLAYERGROUP *aPlayerGroup,
 ...
 */
PLAYERLISTENTRY *
playerlist_get (PLAYERLIST *aPlayerList, int pos)
{
  PLAYERLISTENTRY *aEntry;
  int counter;
 ...
 */
PLAYERLISTENTRY *
playerlist_getentry (PLAYERLIST *aPlayerList, PLAYERCARD *aPlayerCard)
{
  PLAYERLISTENTRY *aEntry;
...
/**
 * gibt den Eintrag der Liste zurueck der den Spieler mit der gesuchten id
 * enthält returns the entry which contains playercard with given id
 */
PLAYERLISTENTRY *
playerlist_getwithid (PLAYERLIST *aPlayerList, int pId)
{
  PLAYERLISTENTRY *aEntry;
...
/**
 * Groesse einer Spielerkarte erweitern (erweitert die Größe der Arrays deren
 * Größe auf der Rundenanzahl basiert) extend the size of a playercard (extend
 * all arrays which size is based on rounds)
 */
void
playercard_setReservedRounds (PLAYERCARD *aPlayerCard, int reservedRounds)
{
  // no need for reallocate
  if (reservedRounds <= aPlayerCard->reservedRounds)
    return;

  if ((aPlayerCard->opponents = (int *)realloc (aPlayerCard->opponents,
                                                sizeof (int) * reservedRounds))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERCARD);
  else
    memset (&(aPlayerCard->opponents[aPlayerCard->reservedRounds]), -1,
            sizeof (*(aPlayerCard->opponents)) * reservedRounds
                - aPlayerCard->reservedRounds);

  if ((aPlayerCard->results = (char *)realloc (aPlayerCard->results,
                                               sizeof (char) * reservedRounds))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERCARD);
  else
    memset (&(aPlayerCard->results[aPlayerCard->reservedRounds]), ' ',
            sizeof (*(aPlayerCard->results)) * reservedRounds
                - aPlayerCard->reservedRounds);

  if ((aPlayerCard->colors
       = (int *)realloc (aPlayerCard->colors, sizeof (int) * reservedRounds))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERCARD);
  else
    memset (&(aPlayerCard->colors[aPlayerCard->reservedRounds]), 0,
            sizeof (*(aPlayerCard->colors)) * reservedRounds
                - aPlayerCard->reservedRounds);

  if ((aPlayerCard->floaterhistory = (int *)realloc (
           aPlayerCard->floaterhistory, sizeof (int) * reservedRounds))
      == NULL)
    fprintf (stderr, L_MEMERROR_PLAYERCARD);
  else
    memset (&(aPlayerCard->floaterhistory[aPlayerCard->reservedRounds]), 0,
            sizeof (*(aPlayerCard->floaterhistory))
                * (reservedRounds - aPlayerCard->reservedRounds));
 ...
 */
void
playercard_setOpponent (PLAYERCARD *aPlayerCard, int aRound, int opponentId)
{
 ...
 */
void
playercard_setResult (PLAYERCARD *aPlayerCard, int aRound, char result)
{
  playercard_setReservedRounds (aPlayerCard, aRound);
 ...
 */
void
playercard_setColor (PLAYERCARD *aPlayerCard, int aRound, int color)
{
  playercard_setReservedRounds (aPlayerCard, aRound);
 ...
 */
void
playercard_setRound (PLAYERCARD *aPlayerCard, int aRound, int opponentId,
...
/**
 * Vertauscht die Spielerkarten die an den uebergebenen Adressen gespeichert
 * sind swaps th playercards which are stored on given addresses
 */
void
playercard_swap (PLAYERCARD **aPlayerCard, PLAYERCARD **anotherPlayerCard)
{
 ...
 */
void
playerlist_moveall (PLAYERLIST *sourcePlayerList, PLAYERLIST *targetPlayerList)
{
 ...
 */
void
pairinglist_moveall (PAIRINGLIST *sourcePairingList,
 ...
 * Ueberprüft NICHT! ob der Eintrag wirklich aus dieser Liste stammt -> das
 * kann in einem segmentation fault resultieren
 */
void
playerlist_movedown (PLAYERLIST *aPlayerList, PLAYERLISTENTRY *aEntry)
{
 ...
 */
void
playerlist_movebefore (PLAYERLIST *aPlayerList, PLAYERLISTENTRY *aEntry,
                       PLAYERLISTENTRY *anotherEntry)
{
  if (aEntry == NULL || anotherEntry == NULL || aEntry == anotherEntry
      || aEntry->next == anotherEntry)
    return;

  // eventuell hier statt dessen vertauschfunktion playercard_swap verwenden

  /* Zuerst den eintrag an seiner aktuellen position ausklinken (ohne ihn zu
   * loeschen) */
  if (aPlayerList->first == aEntry)
    aPlayerList->first = aEntry->next;
  else
 ...
 */
void
playerlist_movelast (PLAYERLIST *aPlayerList, PLAYERLISTENTRY *aEntry)
{
  if (aEntry == NULL || aEntry->next == NULL)
    return;
...
/**
 * Teilt eine Spielerliste ab einem eintrag und kopiert die ueberzaehligen
 * spieler ans ende einer anderen (leeren) Liste
 */
void
playerlist_split (PLAYERLIST *aPlayerList, PLAYERLIST *anotherPlayerList,
                  int pos)
{
  ...
  /* vorgaenger als ende markieren, falls kein vorgaenger dann erste liste =
   * leere liste */
 ...
 */
void
pairinglist_movedown (PAIRINGLIST *aPairingList, PAIRINGLISTENTRY *aEntry)
{
  if (aEntry == NULL || aEntry->next == NULL)
    return;
  // eventuell hier statt dessen vertauschfunktion playercard_swap verwenden
