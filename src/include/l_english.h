...
#define L_MEMERROR_PAIRINGLISTENTRY                                           \
  "Not enough memory for new entry in pairing list.\n"
#define L_MEMERROR_PAIRINGLIST "Not enough memory for new pairing list.\n"

#define L_EVALERROR_OPPONENTNOTFOUND                                          \
  "Opponent of player %d %s in round %d with id %d is not found!\n"
#define L_EVALERROR_OPPONENTNOTMATCH                                          \
  "Player %d %s in round %d has opponent %d %s, but his opponent has id "     \
  "%d!\n"
#define L_EVALERROR_RESULTSNOTMATCH                                           \
  "Results do not match in round %d for players %d %s (%c) - %d %s (%c)!\n"
#define L_EVALERROR_COLORSNOTMATCH                                            \
  "Colors do not match in round %d for players %d %s (%c) - %d %s (%c)!\n"
#define L_EVALERROR_COLORTHREETIMIESINSERIES                                  \
  "Player %d %s has color %c three times in succession (rounds %d-%d).\n"
#define L_EVALERROR_COLORTHREETIMIESMORE                                      \
  "Player %d %s has color %c three times often than color %c in round %d.\n"
#define L_EVALERROR_OPPONENTTWICE                                             \
  "Player %d %s has opponent %d %s twice in round %d and %d!\n"
#define L_EVALERROR_BYEBUTNOPOINT                                             \
  "Player %d %s has bye in round %d but got no point!\n"
#define L_EVALERROR_BYETWICE                                                  \
  "Player %d %s has received a point without playing two times in round %d "  \
  "and %d!\n"
#define L_EVALERROR_TWOPLAYERSWITHBYE                                         \
  "There are 2 players with bye in round %d, %d %s and %d %s!\n"

#define L_INPUTERROR_IGNORINGCHARS                                            \
  "Ignoring chars %d - %d in section %s in line %d.\n"
#define L_INPUTERROR_IGNORINGCHARS_ROUND                                      \
  "Ignoring chars %d - %d in section %s in round %d in line %d.\n"

#define L_INPUTERROR_IGNORINGLINE_WRONGNUMBERFORMAT                           \
  "Ignoring line %d because %s is not in right format \"%s\".\n"
#define L_INPUTERROR_IGNORINGLINE_WRONGNUMBERFORMAT_ROUND                     \
  "Ignoring line %d because %s in round %d is not in right format \"%s\" .\n"
#define L_INPUTERROR_IGNORINGLINE_VALUENOTSET                                 \
  "Ignoring line %d because no %s is set.\n"
#define L_INPUTERROR_IGNORINGLINE_VALUENOTSET_ROUND                           \
  "Ignoring line %d because no %s is set for round %d.\n"
#define L_INPUTERROR_IGNORINGLINE_UNKNOWNCONSTCHAR_ROUND                      \
  "Ignoring line %d because unknown %s '%c' is set for round %d (values "     \
  "%s).\n"
#define L_INPUTERROR_IGNORINGLINE_COLUMNSTOFEW                                \
  "Ignoring line %d because there are to few columns (%d) minimum is (4).\n"
#define L_INPUTERROR_IGNORINGLINE_COLUMNSNOTEQUAL                             \
  "Ignoring line %d because the number of columns (%d) is not equal to "      \
  "column number in first line (%d).\n"
#define L_INPUTERROR_IGNORINGLINE_DUPLICATEPLAYERID                           \
  "Ignoring line %d because the %s (%d) is already asserted to another "      \
  "player.\n"

#define L_INPUTERROR_USINGDEFAULTINT_VALUENOTSET                              \
  "Using default value (%d) for %s in line %d because value is not set.\n"
#define L_INPUTERROR_USINGDEFAULTINT_WRONGNUMBERFORMAT                        \
  "Using default value (%d) for %s in line %d because value is nor in right " \
  "format \"%s\".\n"
#define L_INPUTERROR_USINGDEFAULT_UNKNOWNCONSTSTR                             \
  "Using default value \"%s\" for %s in line %d because value \"%s\" is "     \
  "unknown (values %s).\n"
...
#define L_PARAMERROR_UNKNOWNPARAM                                             \
  "No valid param %s, use option -h for help.\n"
...
#define L_PARAMERROR_FILETOLONG                                               \
  "To long value (%d) of <filename> for option -%c (max %d)."
...
#define L_VERBOSEMSG_HOLDNPPOK                                                \
  "...holding player, he may receive bye and is the only player in the last " \
  "group.\n"
#define L_VERBOSEMSG_HOLDNPPERR                                               \
  "...holding player (this may result in canceling pairings of previous "     \
  "group)\n"
#define L_VERBOSEMSG_SWAPNPPBYE                                               \
  "...swaping player with already paired player %d who can recieve bye\n"
#define L_VERBOSEMSG_SWAPNPPPAIR                                              \
  "...swaping player with already paired player %d who can be paired in "     \
  "this group\n"
...
#define L_VERSIONTEXT_LICENSE                                                 \
  ...
  "law.\nYou may redistribute copies under the terms of the GNU General "     \
...
#define L_HELPTEXT_PARAMS                                                     \
  ...
  " uses <file> as output instead of <stdout>\n"
#define L_HELPTEXT_OPTIONS                                                    \
  "Options:\n -r/R  print the results of previous round to "                  \
  "<stdout>/<stderr> (round>1)\n -t/T  print the current table to "           \
  "<stdout>/<stderr>\n -p/P  print the allotted pairings to "                 \
  "<stdout>/<stderr>\n -d/D  detailed printout of the pairing process to "    \
  "<stdout>/<stderr>\n -e    do not print any errors\n -n    do not allot "   \
  "pairing (-o/p/d were ignored)\n -v    print version and license "          \
  "information\n -h    print this help text\n"
#define L_HELPTEXT_EXAMPLES                                                   \
  "Examples:\n gswiss -T -P           read input from <stdin>, write output " \
  "to <stdout> and\n                        print table and pairings to "     \
  "<stderr>.\n gswiss -t -n -i a.csv  read input from a.csv and print table " \
